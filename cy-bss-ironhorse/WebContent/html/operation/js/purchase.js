...
.config(function($translateProvider) {
 	...
 	.translations('en',{
 		...
 		'NEW.BUTTON':'New',
 		'NEW.TITLE':'New Purchase',
 		'INS.OK': 'Purchase inserted !',
		'UPD.OK': 'Purchase changed !',
		'MODIFY.TITLE':'Change Purchase',
	...
	.translations('it',{
		...
		'DATEFROM.LABEL':'Da',
 		...
 		'NEW.BUTTON':'Nuovo',
 		'NEW.TITLE':'Nuovo Acquisto',
 		'INS.OK': 'Acquisto inserito!',
		'UPD.OK': 'Acquisto modificato !',
		'MODIFY.TITLE':'Modifica Acquisto',
		...
		callRestWs($http,'purchase/'+id+'/get','GET',
			...
			function(response){
					if (response.data.resultCode==RESULT_OK){
						...
						$scope._price=response.data.purchase.price.round(6);
						$scope._priceTot=response.data.purchase.priceTot.round(6);
						...
						$scope._dateClose=response.data.purchase.dateClose==undefined?undefined:dateToStringDDMMYYYY(new Date(response.data.purchase.dateClose));
						$scope._oldId=response.data.purchase.oldId==0?undefined:response.data.purchase.oldId;
						$scope._newId=response.data.purchase.newId==0?undefined:response.data.purchase.newId;
